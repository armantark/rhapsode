...
def test_iliad_11_20_enters_the_settled_acquisition_learning_sequence(
    ...
    # Ten provisioned lines must not arrive as a wall of first lessons: only
    # the default three-line runway is active while the rest stay locked.
    assert [item["mode"] for item in line_session["items"]] == ["acquisition"] * 3
